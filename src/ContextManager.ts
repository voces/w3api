...
export class ContextManager {
	...
	set context( newContext: Context ) { this.currentContext = newContext }
	...
	memoize <U>( fn: () => U ): () => U {
		...
		return (): U => {
			...
			if ( map.has( this.currentContext ) ) return map.get( this.currentContext ) as U;
	...
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	data <T, U extends Array<any>>( fn: ( ...args: U ) => T, memoize = true ): ( ( ...args: U ) => T ) {
	 ...
	 */
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	dataWrapper <T>( generator: () => T, memoize = true ): <U extends any[], G>( fn: ( data: T, ...args: U ) => G ) => ( ...args: U ) => G {
	...
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	simpleFunctionWrapper <A extends Array<any>, B>( generator: ( () => ( ...args: A ) => B ) ) {
