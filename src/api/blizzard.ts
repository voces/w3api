...
export const UnitRemoveBuffsExBJ = (
	...
	const bPos =
		polarity === bj_BUFF_POLARITY_EITHER ||
		polarity === bj_BUFF_POLARITY_POSITIVE;
	const bNeg =
		polarity === bj_BUFF_POLARITY_EITHER ||
		polarity === bj_BUFF_POLARITY_NEGATIVE;
	const bMagic =
		resist === bj_BUFF_RESIST_BOTH || resist === bj_BUFF_RESIST_MAGIC;
	const bPhys =
		resist === bj_BUFF_RESIST_BOTH || resist === bj_BUFF_RESIST_PHYSICAL;
...
export const GetRandomSubGroup = (count: number, sourceGroup: group): group => {
	...
	bj_randomSubGroupChance =
		I2R(bj_randomSubGroupWant) / I2R(bj_randomSubGroupTotal);
...
export const MeleeGetAllyStructureCount = (whichPlayer: player): number => {
	...
	while (true) {
		...
		if (PlayersAreCoAllied(whichPlayer, indexPlayer))
			buildingCount =
				buildingCount + GetPlayerStructureCount(indexPlayer, true);
...
export const MeleeCrippledPlayerTimeout = (): void => {
	...
	while (true) {
		if (bj_crippledTimer[playerIndex] === expiredTimer) if (true) break;
		...
		if (playerScore[index] !== 0) {
			...
			while (true) {
				if (playerScore[index2] !== 0) {
					...
					if (PlayersAreCoAllied(indexPlayer, indexPlayer2)) {
						teamScore[teamCount] =
							teamScore[teamCount] + playerScore[index2];
