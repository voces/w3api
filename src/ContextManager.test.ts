...
describe("ContextManager", () => {
	...
	it("value", () => {
		...
		const setMapName = contextMap((map: Map, name: string): void => {
			map.name = name;
		});
	...
	describe("simpleFunctionWrapper", () => {
		...
		it("handles", () => {
			...
			const getHandle = contextIndexer(
				(id: number): Handle => ({ handleId: id }),
			);
			const getWidget = contextIndexer(
				(id: number): Widget => ({ ...getHandle(), widgetId: id }),
			);
			const getEvent = contextIndexer(
				(id: number): Event => ({ ...getHandle(), eventId: id }),
			);
		...
		it("range", () => {
			...
			const Range = (id: number, min: number, max: number): range => ({
				id,
				min,
				max,
			});
...
describe("injectContextData", () => {
	it("with values", () => {
		...
		const values = contexts.map((context) =>
			cm.with(context, () => injectContextId(3)),
		);
